🌐 <a href="../ko/#/Android/usage">한국어 가이드</a>

This guide explains how to use each ad type.


## List <!-- {docsify-ignore} -->

### [Banner AD](/Android/bannerAd.md)
Explains how to use standard banner ads.


### [BannerWithArticle AD](/Android/bannerWithArticleAd.md)
Explains how to use the BannerWithArticleView ad.


### [Native Ad](/Android/nativeAd.md)
Explains how to use the NativeAdView ad.


### [Native Loader Ad](/Android/nativeLoaderAd.md)
Explains how to use the NativeLoaderAdView ad.


### [Interstitial AD](/Android/interstitailAd.md)
Explains how to use interstitial ads.


### [Ending AD](/Android/endingAd.md)
Explains how to use ending ads.


### [Newsfeed AD](/Android/newsfeedAd.md)
Explains how to use newsfeed banner ads.


### [FreePass AD](/Android/freePassAd.md)
Explains how to use FreePass ads.


### [PointBanner AD](/Android/pointBannerAd.md)
Explains how to use point banner ads.


### [MultiPointBanner AD](/Android/multiPointBannerAd.md)
Explains how to use multi point banner ads.


### [WhoWhoPointBannerBridge](/Android/whowhoPointBannerBridge.md)
Explains how to use multi point banner ads.


### [Reward AD](/Android/rewardAd.md)
Explains how to use reward ads.

### [Smart AD](/Android/smartAd.md)
Explains how to use smart ads (category targeting).

### [Frame AD](/Android/frameBannerAd.md)
Explains how to use frame ads.

### [SPLASH AD](/Android/splashAd.md)
Explains how to use splash ads.

### [HybridBannerBridge](/Android/hybridBannerBridge.md)
Explains how to use hybrid banner bridge ads.

### [EndingBannerVoew AD](/Android/endingBannerAd.md)
Explains how to use ending banner ads.
