...
### 광고 호출방법 1 - MobwithNativeAdViewRender 활용 (추천)

**1.3.10** 버전부터 새롭게 추가된 방식으로 기본 UI 및 모비위드 직광고 전용 UI 추가 등의 기능을 지원합니다.
해당 방식의 경우 광고 지면을 할당하고 별도의 InterfaceBuilder(xib)파일을 제공하여 광고 UI를 구성하게 됩니다.
광고 UI 구성 및 광고 로드 방법은 다음을 참고 하시면 됩니다.  

#### Interface Builder(XIB)를 통한 광고 UI 생성 및 UI 연결

먼저 아래 이미지를 참고하여 광고 UI를 구성합니다. 
admob create mediation groups3  
...
`MobwithNativeAdViewRender`의 Function에 대한 설명은 아래 표를 참고 하시면 됩니다.  


| Function                                    | 설명                                                                                                             |
| ------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| func getMediaView() -> UIView?              | 광고 컨텐츠(이미지, 영상 등)을 표시하기 위한 View를 연결 합니다. **(필수)**                                                              |
| func getAdImageView() -> UIImageView?       | 광고 이미지를 표시하기 위한 ImageView를 연결 합니다. 이미지로 내려오는 광고 컨텐츠에 대해 직접 지정이 필요한 경우가 아니라면, 가급적 위 `getMediaView()`사용을 권장 합니다. |
| func getAdLogoImageView() -> UIImageView?   | 광고주 로고 이미지를 표시하기 위한 ImageView를 연결 합니다.                                                                         |
| func getAdTitleLabel() -> UILabel?          | 광고 메인 타이틀을 표시하기 위한 Label을 연결 합니다.                                                                              |
| func getAdDescriptionLabel() -> UILabel?    | 광고 설명을 표시하기 위한 Label을 연결 합니다.                                                                                  |
| func getGoToSiteButton() -> UIButton?       | 광고 랜딩을 위한 Button을 연결 합니다. CTA 타이틀이 노출되는 위치이며, 광고 랜딩의 경우 표시되는 광고 유형 및 상황에 따라 해당 버튼이 직접 사용되지 않는 경우도 있습니다.        |
| func getInfoLogoImageView() -> UIImageView? | 광고 Info 이미지를 표시하기 위한 ImageView를 연결 합니다. **(필수)**                                                               |
...
각 파라메터별 파라메터별 자세한 내용은 아래 표를 참고하시면 됩니다.  
...
| nibNameForDefault  | 광고를 표시하기 위한 UI가 설정된 xib 파일명을 전달하시면 됩니다.                                         |
| nibNameForDirectAd | 모비위드 직광고 전용 xib 파일명을 설정하시면 됩니다. 기본 UI를 공통으로 사용하려는 경우 설정을 하지 않거나 빈값으로 지정하시면 됩니다. |
| bundle             | xib파일을 불러들이기 위한 bundle을 넘겨주시면 됩니다. `nil`로 설정하는 경우 무조건 `Bundle.main`을 사용하게 됩니다.  |
...
- 광고 표시를 위해 `nibNameForDefault`와 `nibNameForDirectAd`로 UI를 불러오지 못하는 경우 무조건 광고 없음('No Fill') 상태를 반환하게 됩니다.

  
  


### 광고 호출방법 2 - 기존의 고정된 UI 활용방식 (비추천)

기존 방식대로 광고를 표시할 UI를 해당 지면에 미리 구현하여 각 구성요소를 전달 하는 방식 입니다.  
...
#### **ADOP BidMad SDK 를 사용하는 경우 주의 사항**
...
- **주의 : MMNativeAdView의 adDelegate는 [MobWithADViewDelegate](/iOS/banner?id=mobwithadviewdelegate)를 사용하셔야 합니다.**
