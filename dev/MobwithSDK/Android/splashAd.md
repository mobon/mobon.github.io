...
Activity 위에 광고를 표시하며, 하단 카드 형식과 전면 형식 두 가지를 지원합니다.
...
MobwithSplashBannerView splashBanner = new MobwithSplashBannerView(this);

// 배너뷰의 리스너를 등록합니다.
splashBanner.

setAdListener(new MobwithSplashBannerView.OnSplashListener() {
    @Override
...
//광고 타입 설정 (true:전면, false:하단 광고)
splashBannerView.setFullScreenSplash(fullRadioBtn.isChecked());
//광고 자동 닫기  (파라미터는 초 단위)
splashBannerView.setAutoDismiss(3);
// 광고를 호출합니다.
splashBanner.loadAd();
```
...
| setFullScreenSplash(boolean fullScreen)                          | 광고 타입 설정 (true:전면, false:하단 광고) |
| setAutoDismiss(Int second)                                       | 광고 자동 닫기  (파라미터는 초 단위)          |
| stop()                                                           | 광고 새로고침 중단                      |
| destroy()                                                        | 광고 리소스 해제                       |
...
```java
...
splashBanner.setAutoDismiss(3); //3초 후 광고 자동 닫기
...
```
...
setAdListener(new iBannerCallback() {
    @Override
